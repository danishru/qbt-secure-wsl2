# Запуск qBittorrent через защищённый и безопасный канал в Docker на Debian внутри WSL2
Этот проект документирует процесс запуска qBittorrent с **защищённым и безопасным каналом связи** в контейнере Docker, работающем на Debian внутри WSL2 на Windows. Такой подход повышает приватность передачи данных, а окружение Docker обеспечивает повторяемость и изоляцию.
...
>
> **Защищённый канал связи** — это технология, позволяющая создать шифрованное соединение между вашим устройством и удалённой точкой доступа, что повышает приватность и защищённость передачи данных. Однако такой канал **не гарантирует абсолютной анонимности** и не освобождает от ответственности: вы обязаны соблюдать применимые законы и правила использования сети.
> 
...
- Развернуть готовый Docker-образ с qBittorrent (например, [binhex/arch-qbittorrent](https://hub.docker.com/r/binhex/arch-qbittorrent)); при необходимости вместо него можно использовать другой образ, который реализует **нужный тип защиты** (защищённый и безопасный канал), чтобы повысить приватность передачи данных и упростить изоляцию сетевого трафика.
...
- **Защищённый и безопасный канал:** запуск qBittorrent через защищённый канал помогает шифровать трафик и снижать утечку сетевых метаданных (включая реальный IP-адрес).
...
- Действительные параметры/профили для защищённого канала связи, совместимые с используемым образом и вашим провайдером.
- **Portainer** для графического администрирования Docker.

## Ограничения
- **Службы, запущенные в WSL2, не будут доступны через NAT (например, при подключении через публикацию портов на роутере или через внешний защищённый туннель).**
...
- **Расположение томов Docker по умолчанию недоступно из Windows через \\\wsl.localhost\\**

> [!NOTE]  
> Подробные и расширенные инструкции, а также обход ограничений и решение возможных проблем доступны на [Wiki проекта](https://github.com/danishru/qbt-secure-wsl2/wiki).
# Быстрый старт

> [!IMPORTANT]
> Быстрый старт предназначен для уверенного пользователя, знакомого с WSL2, Debian и Docker.  
> Здесь приведены минимальные шаги для быстрого и базового развёртывания qBittorrent в Docker на Debian внутри WSL2.
...
### Шаг 3: Развёртывание qBittorrent через Docker Compose
1. **Создайте папку для тома конфигурации**
   ...
   ```bash
   sudo mkdir -p /srv/docker/volumes/qbittorrent_config/_data
   ```

2. **Создайте файл docker-compose.yml**

> [!NOTE]
> Пример ниже приведён для образа `binhex/arch-qbittorrent`. Если вы используете другой образ с **нужным типом защиты**, часть параметров может отличаться (например, `cap_add`, `ports`, `volumes`, переменные `environment`, имена сервисов/контейнеров). Используйте документацию выбранного образа как основной источник настроек.
   ...
   services:
     qbittorrent:
       image: binhex/arch-qbittorrent:latest
   ...
   volumes:
     config:
       ...
       driver_opts:
         type: none
         o: bind
         device: /srv/docker/volumes/qbittorrent_config/_data
   ```
...
3. **Запустите Docker Compose:**  
   ...
   ```bash
   sudo docker compose -p qbittorrent up -d
   ```
---

### Шаг 4: Создание правила Windows Firewall через PowerShell и доступ к qBittorrent
...
1. Чтобы обеспечить доступ к веб-интерфейсу qBittorrent (порт 8080) с других устройств, выполните в PowerShell (от имени администратора):
   ...
   ```
2. Если выбран qBittorrent с **защищённым и безопасным каналом**, разместите настройки этого канала в папке **соответствующего типа защиты** внутри volume `config` (например, `/open` или `/guard`, в зависимости от выбранного типа).
...
Этот быстрый старт поможет вам развернуть qBittorrent в Docker на Debian внутри WSL2 минимальными усилиями. Более тонкая настройка и дополнительные функции описаны в [Wiki проекта](https://github.com/danishru/qbt-secure-wsl2/wiki).
